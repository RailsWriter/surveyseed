    ...
    Survey.where("CountryLanguageID = ? AND SurveyGrossRank <= ?", @usercountry, 500).order( "SurveyGrossRank" ).each do |survey|
      ...
      if @foundtopsurveyswithquota == false then  #3 false means not finished finding top surveys
        ...
        if ( ((survey.CountryLanguageID == 5) &&        
          ...
          (( survey.QualificationIndustriesPreCodes.empty? ) || ( survey.QualificationIndustriesPreCodes == [ "ALL" ] ) || (( user.industries & survey.QualificationIndustriesPreCodes).empty? == false)) &&
          ...
          @_industries = (( survey.QualificationIndustriesPreCodes.empty? ) || ( survey.QualificationIndustriesPreCodes == [ "ALL" ] ) || (( user.industries & survey.QualificationIndustriesPreCodes).empty? == false))
...
#        puts
    ...
    if user.country=="9" then 
      @RFGAdditionalValues = '&rid='+@rid+'&country=US'+'&postalCode='+user.ZIP+'&gender='+user.gender+'&householdIncome='+@RFGHhi+'&employment='+@RFGEmployment+'&educationUS='+@RFGEducationUS+@RFGchildrenvalue+'&ethnicityUS='+@RFGEthnicity+'&jobTitle='+@RFGJobTitle+'&employmentIndustry='+@RFGPindustry
    else
      if user.country=="6" then
          @RFGAdditionalValues = '&rid='+@rid+'&country=CA'+'&postalCode='+user.ZIP+'&gender='+user.gender+'&educationCA='+@RFGEducationCA+'&householdIncome='+@RFGHhi+'&employment='+@RFGEmployment+@RFGchildrenvalue+'&jobTitle='+@RFGJobTitle+'&employmentIndustry='+@RFGPindustry
      else
      end
    end    
